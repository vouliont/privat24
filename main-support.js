...
class App {
  constructor() {
    _defineProperty(this, "storage", new PermanentStorage());
  ...
  initialize() {
    ...
    this.config = Object.assign({}, App.initialConfig); // read previous state
  ...
  applyConfig(config) {
    const configurationForm = this.configurationScene.querySelector('.configuration-form'); // Set wagon number

    const wagonEl = configurationForm.querySelector('#wagon');
    wagonEl.value = config.wagonNumber; // Set ticket amount

    configurationForm.querySelector('input[name="tickets"]:checked').checked = false;
    configurationForm.querySelector(`input[name="tickets"][value="${config.ticketAmount}"]`).checked = true; // Set is bus
  ...
  emulate(state) {
    ...
    let now = new Date();
    const lastEmulationTime = state.lastEmulationTime ? state.lastEmulationTime : now; // Initialize current ticket
    ...
    for (let i = 0; i < App.previousTicketAmount; i++) {
      this.ticketContainer.appendChild(this.ticketTemplate.cloneNode(true));
      let ticketEl = this.ticketContainer.lastElementChild;
      let date = new Date(now.getFullYear(), now.getMonth(), now.getDate() - (i + 1), Utils.random(8, 22), Utils.random(0, 59), Utils.random(0, 59));
    ...
    function initializeTicket({
      ...
      if (isUsed) makeUsed();else updateEstimateLoop();
...
_defineProperty(App, "previousTicketAmount", 2);

_defineProperty(App, "initialConfig", {
  wagonNumber: '',
  ticketAmount: 1,
  isBus: false
});
...
class PermanentStorage {
  ...
  getState() {
    let lastEmulationTime = parseInt(localStorage.getItem('lastEmulationTime')); // if the ticket time is up
